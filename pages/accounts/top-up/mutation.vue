<template>
  <div class="ant-transaction--mutation">
    ...
    <a-card class="b-solid b-radius b-shadow mt-16">
      <a-table :columns="columns" :dataSource="mutasi" :rowKey="id" :pagination="pagination">
        <span slot="tanggal_mutasi" slot-scope="text, record">{{ moment(record.tanggal_mutasi).format("L") }}</span>
        ...
        <span slot="saldo" slot-scope="text, record">{{ record.saldo | currency }}</span>
...
const columns = [
  ...
  { title: "Saldo", className: 'column-money', dataIndex: "saldo", key: "saldo", scopedSlots: { customRender: 'saldo' } },
